...
if __name__ == '__main__':
	if len(argv) < 2:
		print('Usage: update_coords [input file] [output file] [previous input file]')
		exit()
	else:
		in_file = make_PWfile_obj.make_PWfile_obj(argv[1])
		out_filename = argv[2]
		prev_in_file = make_PWfile_obj.make_PWfile_obj(argv[3])

    ##The calculation done in the previous step is needed
    ##to determine which update is done.
	prev_calculation = prev_in_file.calculation
